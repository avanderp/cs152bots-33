        ...
        if self.state in STATE_TO_EMOJI_OPTIONS and len(current_state_emoji_options):
            ...
            for action in actions:
                if action == ModeratorAction.MUTE_POSTER_TO_REPORTER:
                    self.client.note_in_channel_mute_poster_to_reporter(self.message, self.message.author.name, self.client.poster_to_reporter[self.message.author.name]) # TODO: pass in parameters
